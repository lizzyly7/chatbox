...
class Seq2seq(object):
	...
	def __init__(self, w2i_target, useTeacherForcing=True, useAttention=True, useBeamSearch=1):
	
		self.build_inputs()
		
		with tf.variable_scope("encoder"):
		
			encoder_embedding = tf.Variable(tf.random_uniform([config.VOCAB_SIZE, config.EMBEDDING_DIM]), dtype=tf.float32, name='encoder_embedding')
		...
		with tf.variable_scope("decoder"):
			...
			decoder_embedding = tf.Variable(tf.random_uniform([config.VOCAB_SIZE, config.EMBEDDING_DIM]), dtype=tf.float32, name='decoder_embedding')
					
			with tf.variable_scope("gru_cell"):
				decoder_cell = tf.nn.rnn_cell.GRUCell(config.EMBEDDING_DIM)
			...
			tokens_eos = tf.ones([config.BATCH_SIZE], dtype=tf.int32, name='tokens_EOS') * w2i_target["<eos>"]
			...
			W = tf.Variable(tf.random_uniform([config.HIDDEN_SIZE, config.VOCAB_SIZE]), dtype=tf.float32, name='decoder_out_W')
			b = tf.Variable(tf.zeros([config.VOCAB_SIZE]), dtype=tf.float32, name="decoder_out_b")
