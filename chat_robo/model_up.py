...
class Seq2se(object):
    ...
    def build_in(self):
        self.seq_inputs = tf.placeholder(shape=(1, None), dtype=tf.int32, name='seq_inputs')
        self.seq_inputs_length = tf.placeholder(shape=(1,), dtype=tf.int32, name='seq_inputs_length')
        self.seq_targets = tf.placeholder(shape=(1, None), dtype=tf.int32, name='seq_targets')
        self.seq_targets_length = tf.placeholder(shape=(1,), dtype=tf.int32, name='seq_targets_length')
    ...
    def __init__(self, w2i_target, initializer, useTeacherForcing=True, useAttention=True, useBeamSearch=1):
        ...
        with tf.variable_scope("decoder"):
            ...
            tokens_go = tf.ones([1], dtype=tf.int32, name='tokens_GO') * w2i_target["<sos>"]
            ...
            with tf.variable_scope("gru_cell"):
                ...
                if useAttention:
                    if useBeamSearch > 1:
                        ...
                        tiled_decoder_initial_state = decoder_cell.zero_state(batch_size=1 * useBeamSearch, dtype=tf.float32)
                        ...
                        decoder_cell = tf.contrib.seq2seq.AttentionWrapper(decoder_cell, attention_mechanism)
                        decoder_initial_state = decoder_cell.zero_state(batch_size=1, dtype=tf.float32)
