...
def get_batch(docs_source, w2i_source, docs_target, w2i_target, batch_size):
    ...
    for p in ps:
        ...
        target_seq = [w2i_target[w] for w in docs_target[p]] + [w2i_target["_PAD"]] * (max_target_len - 1 - len(docs_target[p])) + [
            w2i_target["_EOS"]]
...
if __name__ == "__main__":
    ...
    w2i_target ={'<unk>': 0, '<sos>': 1, '<pad>': 2, '<eos>': 3, '的': 4, '是': 5, '吗': 6, '八卦': 7, '了': 8, '有没有': 9, '你': 10, '都': 11, '有': 12, '人': 13,
                 '在': 14, '我': 15, '就': 16, '会': 17, '什么': 18, '不': 19, '要': 20, '台湾': 21, '好': 22, '去': 23, '跟': 24, '很': 25, '被': 26, '啊': 27, '说': 28, '没': 29, '也': 30}
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for batch in range(batches):
            ...
            feed_dict = {
                model.seq_inputs: [[30, 20, 20, 17, 26, 8, 5, 6, 4, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2]],
                model.seq_inputs_length: [9],
                model.seq_targets: [[6, 30, 20, 10, 263, 10, 22, 6, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3]],
                model.seq_targets_length: [9]
            }
